...
## [Mac 0.1.5 / AIUI 1.0.1] - 2026-08-12

### Fixed

- 移除 AIUI 版的二次图片缩放。Mac 现在直接把原图一次处理到最终“小/中/大”像素尺寸，并合成为完整 448×352 画面。
- 位置、透明度、显示/隐藏均在 Mac 合成阶段完成；AIUI 只按 448×352 原尺寸显示完整 PNG 或 GIF 帧。
- 切换大小时会重新从源图生成对应的 96/140/190 像素结果，发送按钮会等待新处理完成，避免误发旧尺寸缓存。

### Changed

- 新安装默认改为“自然绿色”和 100% 透明度，减少量化和低对比度造成的模糊观感；8/16 级量化与抖动仍可手动选择。
- Mac 界面明确标注“0.1.5 AIUI版”；AIUI 包版本更新为 1.0.1。
- GitHub 使用独立 `aiui-v1.0.1` 标签和 AIUI 专用文件名发布，不覆盖或删除 v0.1.4 及以前版本。

### Verified

- AIUI 检查 2/2、官方 AIX 格式验证、Rust 定向测试 6/6、Mac 前端构建和 Tauri release 打包通过。
- RG-glasses 实机包中的 `display_frame.png` 为 448×352；眼镜截图中的 AIUI 显示区域与该文件逐像素比较差异为 0。
- 实机索引只保留正式 Agent ID，版本为 1.0.1；验证素材为程序生成小人，不含私人照片。
