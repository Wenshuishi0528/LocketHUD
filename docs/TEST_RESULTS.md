...
| Show/hide and persistence | Device pass | First Android Back temporarily hid the portrait without persisting the hidden state. Relaunching from the background and force-stop/cold restart both restored the same private portrait and matching screenshot hash. Twenty alternating ADB debug visibility updates kept the same process and produced no crash. |
